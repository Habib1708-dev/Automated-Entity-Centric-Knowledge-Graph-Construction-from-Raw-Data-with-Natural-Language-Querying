...
from kgbuilder.importer import get_driver
...
SCHEMA = TextSchema(
    entity_types=[EntityType(name="Product", description="a product"), EntityType(name="Problem", description="a defect")],
    fact_types=[FactType(predicate="HAS_PROBLEM", subject_type="Product", object_type="Problem", description="product has defect")],
)
...
def scripted(prompt, schema, model=None, use_cache=True):
    ...
    if "wobbles badly" in prompt:
        return ChunkExtraction(triples=[
            RawTriple(subject="Table", subject_type="Product", predicate="HAS_PROBLEM", object="wobbles", object_type="Problem", evidence="The Table wobbles badly"),
            # hallucinated quote: must be rejected
            RawTriple(subject="Table", subject_type="Product", predicate="HAS_PROBLEM", object="cracked leg", object_type="Problem", evidence="the leg cracked in half"),
        ])
    if "scratched surface" in prompt:
        return ChunkExtraction(triples=[
            RawTriple(subject="Table", subject_type="Product", predicate="HAS_PROBLEM", object="scratched surface", object_type="Problem", evidence="a scratched surface"),
        ])
    return ChunkExtraction(triples=[
        RawTriple(subject="Tables", subject_type="Product", predicate="HAS_PROBLEM", object="wobble", object_type="Problem", evidence="The Tables wobble"),
    ])
...
def test_full_pipeline(driver, data_dir, tmp_path, monkeypatch):
    ...
    (data_dir / "extra.json").write_text(json.dumps({"items": [{"id": 1, "meta": {"a": 2}}]}), encoding="utf-8")
    gold = tmp_path / "gold.json"
    gold.write_text(json.dumps([{"subject": "Table", "predicate": "HAS_PROBLEM", "object": "wobbles"}]), encoding="utf-8")
    ...
    assert driver.execute_query("MATCH (:Document)-[:ABOUT]->(:Product {product_id:'P1'}) RETURN count(*) AS c")[0][0]["c"] == 1
    assert driver.execute_query("MATCH (:Entity)-[:REFERS_TO]->(:Product) RETURN count(*) AS c")[0][0]["c"] == 1
...
def test_verify_rejects_ungrounded_and_off_schema():
    ...
    ok = RawTriple(subject="Table", subject_type="Product", predicate="HAS_PROBLEM", object="wobbles", object_type="Problem", evidence="The Table wobbles")
...
def test_chunking_and_json_staging(tmp_path):
    ...
    src = tmp_path / "x.json"
    src.write_text(json.dumps([{"id": 1, "o": {"k": "v"}, "l": [1, 2]}, {"id": 2, "o": {"k": "w"}, "l": []}]), encoding="utf-8")
