...
def validate_graph(
    ...
    if plan:
        for rule in plan.nodes:
            ...
            add(f"domain:{rule.label} count", exp is None or n == exp, f"{n} nodes" + (f", expected {exp}" if exp is not None else ""), "structure")
            uniq = _scalar(driver, f"MATCH (n:{cypher_ident(rule.label)}) WITH n.{cypher_ident(rule.unique_column)} AS k, count(*) AS c WHERE c > 1 RETURN count(k)")
            add(f"domain:{rule.label} unique keys", uniq == 0, f"{uniq} duplicated keys", "structure")
        isolated = sum(
            _scalar(driver, f"MATCH (n:{cypher_ident(r.label)}) WHERE NOT (n)--() RETURN count(n)") for r in plan.nodes
        )
        # unreferenced rows are a property of the data, not a build error, so this is reported but never fails
        add("domain: isolated nodes (informational)", True, f"{isolated} domain nodes have no relationships", "structure")
        metrics["domain_nodes"] = sum(_scalar(driver, f"MATCH (n:{cypher_ident(r.label)}) RETURN count(n)") for r in plan.nodes)
    ...
    if docs:
        orphan_chunks = _scalar(driver, "MATCH (c:Chunk) WHERE NOT (c)-[:PART_OF]->(:Document) RETURN count(c)")
        add("lexical: every chunk belongs to a document", orphan_chunks == 0, f"{orphan_chunks} orphan chunks", "structure")
        empty_docs = _scalar(driver, "MATCH (d:Document) WHERE NOT (:Chunk)-[:PART_OF]->(d) RETURN count(d)")
        add("lexical: every document has chunks", empty_docs == 0, f"{empty_docs} empty documents", "structure")
        unlinked = _scalar(driver, "MATCH (d:Document) WHERE NOT (d)-[:ABOUT]->() RETURN count(d)")
        add("link: every document is linked to the domain graph", unlinked == 0, f"{unlinked} of {docs} documents unlinked", "structure")
    ...
    if entities:
        ...
        add("provenance: every entity is mentioned in a chunk", no_mention == 0, f"{no_mention} entities without a source chunk", "provenance")
        ...
        add("provenance: every fact has chunk_id and evidence", not no_prov, f"{len(no_prov)} of {len(facts)} facts lack provenance", "provenance")

        chunk_text = {r["id"]: norm(r["t"]) for r in driver.execute_query("MATCH (c:Chunk) RETURN c.chunk_id AS id, c.text AS t")[0]}
        bad_ev = [f for f in facts if f["cid"] and f["ev"] and norm(f["ev"]) not in chunk_text.get(f["cid"], "")]
        add("provenance: evidence quotes exist in their chunk", not bad_ev, f"{len(bad_ev)} of {len(facts)} quotes not found", "provenance")
        ...
        if schema:
            ...
            add("consistency: every fact conforms to the text schema", not bad_type, f"{len(bad_type)} facts violate the schema", "consistency")
            types_ok = _scalar(driver, "MATCH (e:Entity) WHERE e.type IN $ok RETURN count(e)", ok=sorted(schema.entity_names()))
            add("consistency: every entity type is in the schema", types_ok == entities, f"{entities - types_ok} entities of unknown type", "consistency")

        dupes = _scalar(
            driver,
            "MATCH (e:Entity) WITH e.type AS t, toLower(e.name) AS n, count(*) AS c WHERE c > 1 RETURN count(n)",
        )
        add("resolution: no exact-duplicate entities remain", dupes == 0, f"{dupes} duplicate name groups", "consistency")
        ...
        if gold_path and Path(gold_path).exists():
            ...
            found = {(frozenset(norm(a) for a in [f["sname"], *f["sal"]]), f["pred"], frozenset(norm(a) for a in [f["oname"], *f["oal"]])) for f in facts}
            hit = 0
            for g in gold:
                if any(norm(g["subject"]) in s and g["predicate"] == p and norm(g["object"]) in o for s, p, o in found):
                    hit += 1
            recall = hit / len(gold) if gold else 1.0
            metrics["gold_recall"] = recall
            add("accuracy: gold recall", recall >= 0.5, f"recall {recall:.0%} ({hit}/{len(gold)})", "accuracy")
